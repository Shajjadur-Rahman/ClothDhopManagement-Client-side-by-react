...
import {allOrder, payDueAmount2} from '../../../../Services/Actions/ClientOrderActions'
import PaidDueAmount from '../PaidDueAmount/PaidDueAmount'
import { Modal } from '../../../../UI/Modal/Modal';
import './ClientAllOrders.css'
...
const ClientAllOrders = ({
    ...
    useEffect(() => {
        document.title = `${name}'s all orders`
    }, [name])
    ...
    const onSubmitHandler = event => {
        ...
        console.log(orderObj.due_amount, paid)
        if(paid > orderObj.due_amount){
            setError('Invalid amount ! Paid amount should be lessthan or equal to due amount !')
        }else{
            payDueAmount2(paid, orderObj.id)
            setError('')
            modalHide()
        }
    }
                        ...
                        <tbody>
                            {
                                orders.length !== 0 ? orders.map((order, index)=> (
                                    ...
                                    <tr key={order.id} style={{background: 'rgb(54, 55, 75)'}}>
                                        ...
                                        <td style={{color: '#fff'}} colSpan="6">{new Date(order.timestamp).toLocaleString()} {order.due_amount > 0 && <button className="paid__btn" onClick={() => paidDueAmount(order)}>Pay</button>}</td>
                                    </tr>
                                    ...
                                    <tr style={{background: order.due_amount > 0 ? '#ce4785' : ''}}>
                                        <td colSpan="4" style={{color: order.due_amount > 0 ? '#fff' : '#000'}}><strong>Total</strong></td>
                                        <td colSpan="2" style={{color: order.due_amount > 0 ? '#fff' : '#000'}}><strong>{getTotal(order.orderItems)} {"-"} {order.paid_amount.toFixed(2)} {"="} {order.due_amount.toFixed(2)}</strong></td>
                                        <td style={{color: order.due_amount > 0 ? '#fff' : '#000'}}><strong>Due amount : {order.due_amount.toFixed(2)}</strong></td>
                                    </tr>
                                    <br/>
                                    <br/>
...
const mapStateToProps = state => ({
    loading: state.ClientOrder.loading,
    orders: state.ClientOrder.orders
})
export default memo(connect(mapStateToProps, {allOrder, payDueAmount2})(ClientAllOrders))